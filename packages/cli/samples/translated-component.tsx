import { useTranslation } from "@terminus/t-console-framework"
// 한글 테스트용 모음

// 1. 함수 선언문 + JSX 안에 직접 한글
function HelloComponent() {
  const { t } = useTranslation()
  return <div>{t("안녕하세요2")}</div>
}
...
const AnotherComponent = () => {
  const { t } = useTranslation()
  return <span>{t("안녕하세요 스팬입니다")}</span>
}
...
function ConditionalComponent({ isKorean }) {
  const { t } = useTranslation()
  return <div>{isKorean ? t("안녕하세요") : "Hello"}</div>
}
...
const MultipleStringLiterals = () => {
  const { t } = useTranslation()
  const greeting = t("안녕하세요")
  const message = t("반갑습니다")
...
function ComplexConditional({ isMorning, user }) {
  ...
  // 삼항 연산자 + 템플릿 리터럴
  // 한국어 / 영어 혼합
  const timeStr = isMorning ? t("아침") : t("오후")
  return (
    <div>
      {t("{{user.name}}님, 지금은 {{timeStr}}입니다. Hello!", {
        "user.name": user.name,
        timeStr: timeStr,
      })}
    </div>
...
function DoubleAttributeComponent() {
  const { t } = useTranslation()
  return (
    <section title={t("안녕하세요 타이틀")} aria-label={t("한글 라벨")}>
      <p>{t("한글 paragraph")}</p>
    </section>
  )
}
...
const MixedTemplate = ({ user, date }) => {
  ...
  return (
    <div>
      {t("{{user.name}}님, 오늘은 {{date}} 입니다. Have a good day!", {
        "user.name": user.name,
        date: date,
      })}
    </div>
